...
/* We MUST enumerate the directories to find the current directory. Otherwise,
 if we use the GetResource method, the top directory will not recognize the
 CardDAV entries with the hasDirectory method. */
...
function SCCopyAddressBook(sourceAB, destAB) {
    ...
    while (nodes.hasMoreElements()) {
        ...
        childCards = SCGetChildCards(currentList);
        for (i = 0; i < childCards.length; i++)
